...
function buildWorkbookModel(runRows, auditRows, traceAuditRows, frameCorrelationRows, ledgerRows) {
  ...
  const workbookRuleRows = [
    ...
    ["Optimization workflow source", "Agents optimizing Flight Alert must read this Performance Notes sheet before starting.", "AGENTS.md keeps fundamental app/product rules and a pointer here; the detailed optimization loop, comparison policy, benchmark lanes, Pro/MCP usage, warning audits, and rejection logging live in this workbook."],
    ["Master comparisons", "Do not use master comparisons as a standing optimization workflow.", "Use master only when the user explicitly requests it or an obvious visual glitch needs a targeted reality check. Keep C:\\Users\\h\\AndroidStudioProjects\\FlightAlert-master-compare read-only."],
    ["Normal comparison baseline", "Use the pushed post-master-inventory checkpoint or newer accepted checkpoint as the normal comparison baseline.", "The historical one-huge-file checkpoint is checkpoint-one-huge-file-20260621-fair-master. Do not compare against half-tested local experiments."],
    ["Hotspot workflow", "Start from profiling/detail evidence, audit the hot function with Android Studio MCP, make the smallest behavior-preserving implementation change, build, warning-audit, and validate on a physical phone.", "Use Pro for high-value independent strategy when available, but accept Pro only when readback is a complete substantive answer."],
    ...
    ["Android Studio MCP audits", "Renderer/performance edits require a function-specific Android Studio MCP audit before speculative hot-code changes.", "Use search/get_symbol_info/get_file_problems(errorsOnly:false) when available, or the bridge wrapper command. Build with android-studio-mcp-call.mjs --build-project --timeout 180000 after edits."],
    ["Warnings", "Android Studio audits must collect warnings, not only errors.", "Fix behavior-preserving relevant warnings, suppress intentional/irrelevant inspections, or record why legacy monolith debt remains. Ignore naming-convention warnings unless they are actual errors."],
    ["Pro mode", "Use Pro for high-value optimization advice when healthy, but keep it advisory.", "Do not send repeated tiny asks. Use guidance only when proResponseState.complete is true with a substantive answer; treat stubs, progress-only replies, stale tool activity, and rate limits as non-guidance."],
    ["Bridge failures", "Report Pro Agent Bridge or Android Studio MCP bridge failures to the Use Advisory Bridge agent/thread.", "Include exact command/tool shape, cwd, timeout, observed result, expected result, and local fallback. Do not develop bridge tooling in this app repo."],
