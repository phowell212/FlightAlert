    ...
    if (summary && await exists(summary)) {
      ...
      if (lines.length >= 2) {
        ...
        runRows.push({
          ...
          packageCompileEvidence: route.package_compile_evidence || "",
          ...
          displayRefreshEvidence: route.display_refresh_evidence || "",
...
function buildWorkbookModel(runRows, auditRows, traceAuditRows, frameCorrelationRows, ledgerRows) {
  ...
  const runData = [runHeaders, ...enrichedRunRows.map((row) => [
    ...
    safeText(row.packageCompileEvidence, 2200),
    row.batteryLevel,
    row.batteryTempC,
    row.batteryStatus,
    row.batteryPlugged,
    ...
    safeText(row.displayRefreshEvidence, 2200),
