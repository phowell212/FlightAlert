...
- Use this as a density heuristic, then confirm with a quick live-feed sanity check before treating a run as comparable. Do not compare a sparse overnight US run against a dense European daytime run as an optimization win or loss unless the traffic count/visual density is intentionally part of the scenario.
...
- `15:00-22:00 UTC`: US is usually the better dense-traffic target. This is US late morning through evening and Europe evening into night. Prefer inland US major-airport targets already allowed by the test rules: Dallas-Fort Worth, Atlanta, Denver, Phoenix, Las Vegas; add Chicago/New York/Los Angeles only when the route-proof envelope and map focus remain land/traffic centered.
