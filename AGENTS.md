...
Before starting or completing an optimization iteration, read the consolidated workbook sheets `Data`, `Charts`, and `Performance Notes`. `Data` contains the `Workbook Tests` block, all parsed `Runs`, `Detailed Audits`, `Trace Audits`, and `Frame Correlations`; it is the metric source of truth. `Charts` contains the user-facing progress graphs for average FPS, P95 FPS, P99 FPS, and Android jank over the active apples-to-apples workbook lane. `Performance Notes` contains the dashboard context, workbook rules, and full optimization ledger. The detailed audit data must preserve the detailed performance-level timing/counter rows from `Debug draw perf`, not only simplified summaries. The frame correlation data must preserve matched per-frame Perfetto/log metrics when frame-token correlation artifacts exist, not only a simplified GO/STOP summary. The optimization ledger carries the historical accepted/rejected/deferred notes that used to live in this file; do not remove those notes when cleaning workbook layout.
...
The user-facing workbook chart must be an apples-to-apples benchmark lane, not a plot of whatever ran last. Keep dirty, missing-clean-git-stamp, diagnostic, video, route-failed, hidden-aircraft/no-aircraft-draw-evidence, layer-isolation, skip-traffic, trace/Perfetto, workload-specific, known-rejected/reverted experiments, uncontrolled thermal/dexopt/ART state, and nonmatching-city/series runs out of `Workbook Tests`; retain them in `Runs` and supporting evidence sheets with explicit exclusion reasons. `Workbook Tests` should use the active comparable workbook-test series matching the latest workbook-test lane, harness execution mode, city, map mode, roads, borders, detail-timing flags, package dexopt state/fingerprint, post-run dexopt state/fingerprint, ART compile mode, controlled expected dexopt state, and controlled dexopt normalization mode. Chart rows must have `git_worktree_dirty=false`, `thermal_status=0`, a controlled package dexopt state/fingerprint, unchanged post-run dexopt, and ART compile mode `InstallDefault` recorded by the perf harness. The approved controlled dexopt lanes are `verify/install-speg` with normalization mode `none`, and explicit `verify/install` with normalization mode `post_install_reset_v1`; never compare those lanes as one population, and never promote `verify/cmdline`, `run-from-apk/unknown`, or unknown package states into `Workbook Tests`. For future chart-grade SplitInstall workbook tests, use the harness controlled preflight gate and require `controlled_preflight_passed=True`; do not start the 60-second workload if thermal or dexopt controls are already invalid. Each optimization iteration should include one roughly minute-budget standardized "workbook test" in the timetable-selected dense US/EU region for that time of day, with full visible UI and aircraft traffic, satellite + roads + borders/labels when relevant, route proof, human-like overlapping pan/zoom, and frame-time/FPS metrics. Prefer no-detail-timing workbook tests for user-facing FPS/p95 trend comparisons; use `TrafficDetailTiming`/`MapDetailTiming` runs as detailed diagnostic evidence in matching detail-on series or supporting sheets. Do not select a best checkpoint or restore point until suspected iterations have been compared with these standardized full workload tests, using repeated comparable rows rather than one lucky run.

Rejected or reverted optimization attempts still require apples-to-apples evidence. For each attempted implementation change, run a diagnostic apples-to-apples comparison against the previous accepted checkpoint or immediately preceding accepted attempt unless preflight rejects before capture. Log the attempt name, baseline artifact, candidate artifact, FPS/p95/p99/jank deltas, route proof, warning/build status, and exact failure reason in `Performance Notes`; then restore the previous accepted app behavior before continuing. Diagnostic comparison rows stay out of the user-facing `Charts` lane unless they satisfy the clean Workbook-test rules above, but they must remain in `Data` and the notes so the iteration history is auditable.
