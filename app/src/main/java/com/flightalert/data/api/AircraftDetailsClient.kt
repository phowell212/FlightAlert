...
class AircraftDetailsClient(private val user_agent: String) {
    ...
    fun fetch_details(
        ...
        val origin = adsb_route?.origin ?: route_codes?.firstOrNull()?.let { fetch_airport(it) }
        val destination = adsb_route?.destination ?: route_codes?.lastOrNull()?.let { fetch_airport(it) }
        ...
        if (clean_callsign.isNotEmpty()) {
            Log.d(
                ...
                "Route lookup hex=$normalized_hex callsign=$clean_callsign " +
                    "adsb=${adsb_route?.route ?: "none"} hexdb=${route?.opt_string_or_null("route") ?: "none"} " +
                    "origin=${origin?.icao ?: "none"} destination=${destination?.icao ?: "none"}"
            )
        }
        val feed_type = airplanes_live?.description
    ...
    private fun details_from_seed(
        ...
        if (seed == null || !seed.has_details) return null
        val registration = first_present(
        ...
        if (listOf(registration, manufacturer, type, type_code, owner, year, operator_code).all { it.isNullOrBlank() }) {
            return null
        }
        return AircraftDetails(
            icao24 = normalized_hex,
    ...
    private fun fetch_airplanes_live_metadata(hex: String): AirplanesLiveMetadata? {
        ...
        fetch_airplanes_live_trace_metadata(hex)?.let { return it }
        return fetch_airplanes_live_rest_metadata(hex)
    }

    private fun fetch_airplanes_live_trace_metadata(hex: String): AirplanesLiveMetadata? {
        val folder = hex.takeLast(2)
        val json = fetch_json("${AirplanesLiveHttp.GLOBE_BASE_URL}/data/traces/$folder/trace_full_$hex.json", browser_headers = true)
            ?: return null
        return json.to_airplanes_live_metadata("Airplanes.Live trace")
    }
...
private fun JSONObject.to_airplanes_live_metadata(source_name: String): AirplanesLiveMetadata {
    return AirplanesLiveMetadata(
        source_name = source_name,
        registration = opt_string_or_null("r"),
        type_code = opt_string_or_null("t"),
        description = opt_string_or_null("desc"),
        owner_operator = opt_string_or_null("ownOp"),
        operator_code = opt_string_or_null("ownOpCode"),
        year = opt_string_or_null("year")?.take(4)
    )
}
